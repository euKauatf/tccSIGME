...
function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [eventsResponse] = await Promise.all([apiClient.get("/event")]);
  ...
  const ordemDosDias = ["Segunda", "Terça", "Quarta", "Quinta", "Sexta"];
  const diasJs = [
    "Domingo",
    "Segunda",
    "Terça",
  ...
  const nomeHoje = diasJs[hojeIndex];
  const indiceHojeNaSemana = ordemDosDias.indexOf(nomeHoje);
  const TotalEventosInscritos = user?.eventos
    ? user.eventos.filter((e) => e.pivot?.status === "inscrito").length
    : 0;
  const TotalEventosSelecionados = user?.eventos
    ? user.eventos.filter((e) => e.pivot?.status === "contemplado").length
    : 0;
  const proximosEventosSelecionados = user?.eventos
    ? user.eventos
        .filter((e) => {
          const indiceEvento = ordemDosDias.indexOf(e.data);
          return (
            indiceHojeNaSemana !== -2 && indiceEvento >= indiceHojeNaSemana
          );
        })
        .sort((a, b) => {
          ...
          <div className="flex-row justify-between hidden w-full gap-6 lg:flex">
            <div className="flex flex-col gap-6 pl-28">
              <div className="flex flex-col divp rounded-[20px] min-h-[170px] mb-30 px-6 bg-emerald-50">
                ...
                <p>
                  Local para adicionar/mudar todas as atividades registradas.
                </p>
                <p className="font-bold linkpage text-zinc-950 btn-link">
                  <a href="/events">Clique aqui para acessar!</a>
                </p>
        ...
        <div className="w-full mt-6">
          <div className="flex flex-col items-center gap-4 lg:hidden">
              ...
              {TotalEventosInscritos > 0 ? (
                <>
                  <p>
                    ...
                    <span className="font-bold">{TotalEventosInscritos}</span>{" "}
                    sorteios!
                  </p>
                  <p className="mt-2 font-bold linkpage text-zinc-950 btn-link">
                ...
                <>
                  <p>
                    Você ainda não está concorrendo a nenhum sorteio de
                    atividade.
                  </p>
                  <p className="mt-2 font-bold linkpage text-zinc-950 btn-link">
            ...
            <div className="flex flex-col divp rounded-[20px] p-6 bg-emerald-50 w-full max-w-md">
              ...
              <p>
                Você está selecionado em{" "}
                <span className="font-bold">{TotalEventosSelecionados}</span>{" "}
                atividades!
              </p>
              <p className="mt-2 font-bold linkpage text-zinc-950 btn-link">
                <a href="/events?filter=selecionados">
                  Clique para ver as atividades que participará!
                </a>
              </p>
            </div>
              ...
              {proximosEventosSelecionados.length > 0 ? (
                <ul className="flex-grow w-full space-y-2">
                  {proximosEventosSelecionados.map((evento) => (
                    <li
                      key={evento.id}
                      className="p-3 text-center rounded-lg shadow-sm divp bg-emerald-50">
                      <div className="font-bold text-emerald-800">
                        {evento.tema}
                      </div>
                      <div className="text-sm font-bold">
                        <p>
                          {evento.data} às {evento.horario_inicio}
                        </p>
                      </div>
                    </li>
                  ))}
              ...
              <div className="flex flex-col divp rounded-[20px] min-h-[170px] mb-30 px-6 bg-emerald-50">
                ...
                {TotalEventosInscritos > 0 ? (
                  <>
                    ...
                    <p className="font-bold linkpage text-zinc-950 btn-link">
                      <a href="/events?filter=pendentes">
                        Veja aqui quais são!
                      </a>
                    </p>
                  </>
