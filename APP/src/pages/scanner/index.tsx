...
function ScannerPage() {
  ...
  const handleScanResult = async (matricula: string) => {
    ...
    if (selectedEventId) {
      try {
        ...
        const errorMessage =
          err.response?.data?.message || "Erro ao registar presença.";
  ...
  const handleStartScanner = () => {
    if (!selectedEventId) {
      setMessage({
        type: "error",
        text: "Por favor, selecione um evento antes de iniciar o scanner.",
      });
      return;
    }
    setMessage(null);
  ...
  return (
    <div className="flex flex-col items-center w-full p-4 font-sans main">
      ...
      <div className="w-full max-w-lg p-6 mt-6 bg-white rounded-lg shadow-lg divp">
        <div className="mb-4">
          <label
          ...
          <div className="w-full dropdown dropdown-bottom">
            ...
            <ul
              tabIndex={0}
              className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full mt-1 max-h-60 overflow-y-auto">
        ...
        {!isScannerActive ? (
          ...
          <div className="p-4 mt-4 border-2 border-gray-300 border-dashed rounded-lg">
