...
// Define os tipos de status que o modal pode ter
...
interface InfoModalProps {
  ...
  title: string;
  message: string;
  status?: ModalStatus; // Status é opcional, padrão 'info'
}

// Mapeia o status para as cores dos ícones e botões do DaisyUI
const statusStyles = {
  success: {
...
function InfoModal({ isOpen, onClose, title, message, status = 'info' }: InfoModalProps) {
  // Efeito para bloquear o scroll
  useEffect(() => {
    if (isOpen) {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="relative w-full max-w-md p-6 mx-4 rounded-lg shadow-xl bg-base-100" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-start gap-4">
          {/* Ícone de Status */}
          <svg xmlns="http://www.w3.org/2000/svg" className={`stroke-current shrink-0 h-8 w-8 ${colorClass}`} fill="none" viewBox="0 0 24 24">
